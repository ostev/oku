...
Peggy is 1.5 metres in front of me, but I need to be within 1 metre
...
// Move forward 4 metres
forward(4)

// Gloat
say("Hee hee hee hee! Take that! I crashed into you!")
```

<YourTurn>
<Goal index={1}>Move to within one metre of Peggy and speak to her.</Goal>
...
In addition to being able to move <DocLink symbol="forward">`forward`</DocLink>, I can also <DocLink symbol="turn">`turn`</DocLink>! Like <DocLink symbol="forward">`forward`</DocLink>, <DocLink symbol="turn">`turn`</DocLink> also takes one parameter: a <DocLink symbol="number">number</DocLink>
indicating how far I should turn in degrees. You use it like this:
...
<Goal index={2}>
    Say hello to Arnold, the bird over in the corner, using these new commands.
</Goal>

```javascript
// 5
// Hint: `turn`, then move `forward`, *then* `say` something
```
